		...
		if( o == OP_CLOSE ){
			...
			***/
			if( dest == F->loop_ptr->next_code ){
				...
				if((pc_1 == OP_TFORLOOP) || (pc_1 == OP_TFORCALL)){
					// TFORLOOP jump back
	  ...
	  case OP_JMP:
		  {
			  ...
			  if (foundInt != NULL){ // break
				  ...
				  F->elsePending = dest;
				  F->elseStart = pc + 2;
			  }else if (GET_OPCODE(idest) == OP_TFORCALL) { // jmp of generic for
				  /*
				  * generic 'for'
				  */
				  int i;
				  //int step;
				  const char *generator, *state, *control;
				  //char *variables[20];
				  char* vname[40];
				  //int stepLen;
					  ...
					  if (!IS_VARIABLE(a+i)) {
						  ...
						  for (i2 = 0; i2 < f->sizelocvars; i2++) {
							  if (f->locvars[i2].startpc == pc + 1) {
								  ...
								  if (GET_OPCODE(F->f->code[f->locvars[i2].endpc-2]) == OP_TFORCALL) {
									  f->locvars[i2].endpc -= 2;
								  }
								  if (GET_OPCODE(F->f->code[f->locvars[i2].endpc-1]) == OP_TFORCALL) {
				  ...
				  for (i=1; i<=c-2; i++) {
					  StringBuffer_addPrintf(str, ",%s",vname[i]);
				  }
				  ...
				  F->intbegin[F->intspos] = a-3;
				  F->intend[F->intspos] = a+c-2;
				  TRY(AddStatement(F, str));
				  F->indent++;
				  break;
...
LOGIC_NEXT_JMP:
	  ...
	  case OP_TFORCALL: //Lua5.1 specific. TODO: CHECK
