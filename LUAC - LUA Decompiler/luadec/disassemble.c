...
void luaU_disassemble(Proto* fwork, int dflag, int functions, char* name) {
	...
	for (pc=0; pc<f->sizecode; pc++) {
	  ...
	  case OP_GETUPVAL:
		  ...
		  if (f->upvalues && b < f->sizeupvalues) {
			sprintf(lend,"%c%d := %s",CC(a),CV(a),getstr(f->upvalues[b]));
		  } else {
			sprintf(lend,"%c%d := U%d",CC(a),CV(a),b);
		  }
		  break;
	  ...
	  case OP_SETUPVAL:
		  sprintf(line,"%c%d U%d",CC(a),CV(a),b);
		  if (f->upvalues && b < f->sizeupvalues) {
			sprintf(lend,"%s := %c%d",getstr(f->upvalues[b]), CC(a),CV(a));
		  } else {
	  ...
	  case OP_LE:
		  {
			  ...
			  sprintf(lend,"if %s %s %s then PC := %d",tmp,a?invopstr(o):opstr(o),tmp2,pc+2);
		  }
		  break;
	  case OP_TEST:
		  {
			  ...
			  sprintf(lend,"if %s%s then PC := %d",c?"not ":"",tmp,pc+2);
		  }
		  break;
	  case OP_TESTSET:
		  {
			  ...
			  sprintf(lend,"if %s%s then %s := %s else PC := %d",c?"":"not ",tmp2,tmp,tmp2,pc+2);
		  }
		  break;
	  case OP_CALL:
	  case OP_TAILCALL:
	  ...
	  case OP_FORLOOP:
		  {
			  ...
			  sprintf(lend,"R%d += R%d; if R%d <= R%d then { PC := %d; R%d := R%d }",a,a+2,a,a+1,pc+sbc+1,a+3,a);
		  }
		  break;
	  case OP_TFORLOOP:
		  {
	  ...
	  case OP_TESTNIL:
		  sprintf(line,"R%d %d",a,sbc);
		  sprintf(lend,"if R%d ~= nil then R%d := R%d; PC := %d", a+1, a, a+1, pc+sbc+1);
		  break;
	  case OP_TFORCALL:
		  sprintf(line,"R%d %d",a,c);
		  if (c>2) {
			  sprintf(tmp,"R%d to R%d", a, a+c-2);
		  } else if (c==0) {
			  sprintf(tmp,"R%d to top",a);
		  } else {
			  sprintf(tmp,"%s","");
		  ...
		  sprintf(lend,"%s := R%d(R%d,R%d)",tmp,a-3,a-2,a-1);
		  break;
	  default:
		  break;
		}
		if (f->sizelineinfo != 0) {
			printf("%5d [%5d]: %-9s %-13s; %s\n",pc,f->lineinfo[pc],luaP_opnames[o],line,lend);
		} else {
	...
	if (f->sizelocvars !=0) {
		int i;
		printf("\n");
		printf("; Local Start End   Name\n");
		for (i=0; i < f->sizelocvars; i++) {
			printf("; %-5d %-5d %-5d %s\n", i, f->locvars[i].startpc-1, f->locvars[i].endpc-1, getstr(f->locvars[i].varname));
		}
	}
	printf("\n\n");
